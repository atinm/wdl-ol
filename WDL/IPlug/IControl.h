...
class IDirectorySelectorControl : public IControl
{
public:
	enum EDirectorySelectorState { kDSNone, kDSSelecting, kDSDone };

	IDirectorySelectorControl(IPlugBase* pPlug, IRECT pR, int paramIdx, IBitmap* pBitmap,
		char* dir = "")     // extensions = "txt wav" for example.
		: IControl(pPlug, pR, paramIdx), mBitmap(*pBitmap),
		mStartDir(dir), mState(kDSNone) {}
	~IDirectorySelectorControl() {}

	void OnMouseDown(int x, int y, IMouseMod* pMod);

	void GetLastSelectedDirectoryForPlug(WDL_String* pStr);
	void SetLastSelectedDirectoryFromPlug(char* file);
...
protected:
	IBitmap mBitmap;
	WDL_String mStartDir, mDir, mExtensions;
	EDirectorySelectorState mState;
};
